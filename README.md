...
| **资产详情** | ECharts K 线（日 / 周 / 月）+ MA5 / MA20 / MA60 + 成交量副图 + 区间缩放；MACD、RSI、KDJ、布林带等技术指标 KPI ⚠️ *历史 K 线目前为合成数据，指标仅供界面演示，勿作决策依据（见「行情数据源」章节）* |
...
- **诚实优先**：`auto` / `real` 模式下真实源拉不到就如实标注 `nosource`（价格 0），**绝不用模拟价冒充真实行情**污染盈亏。只有把数据源模式显式改成 `sim` 才会生成模拟价（离线演示 / 截图用）
...
| 全部 | 内置随机游走模拟器（**仅** `data_source_mode=sim` 时启用） | `sim` | — |

行情带内存缓存和 K 线缓存，避免重复请求打爆上游。每条行情携带 `status`：`ok`（真实源）/ `sim`（模拟器）/ `stale`（真实源但数据偏旧）/ `nosource`（真实源不可达，价格 0）。
...
> ```bash
> HTTP_PROXY=http://127.0.0.1:1082 HTTPS_PROXY=http://127.0.0.1:1082 ./investhub
> ```
> 注意：提醒 Webhook 的出站请求**永不走代理**（SSRF 防护，见「安全」章节），不受此设置影响。

> ⚠️ **已知局限：K 线历史目前是合成的**。实时报价（`price` / `chgPct`）来自上表的真实数据源，但**历史 K 线尚未接入真实日线**——首次查看资产详情时由 `EnsureKline` 用随机游走生成一段以当前真实价收尾的假历史。因此**资产详情页的 K 线形态、MA / MACD / RSI / KDJ / BOLL 指标、以及基于这些指标的 AI 技术面研判都不可作为决策依据**。请只依赖实时报价与持仓盈亏。接入真实历史日线是当前的首要待办。
...
| `kline_cache` | K 线缓存。⚠️ 当前存放的是 `EnsureKline` 合成的历史（非上游真实日线），仅最新一根会被真实报价滚动更新 |
| `alert_rules` | 提醒规则 |
| `alert_events` | 提醒触发记录 |
| `ai_analyses` | AI 分析历史 |
...
**Q：必须联网吗？**
不必须，但断网时**不会**用模拟价冒充真实行情。默认 `auto` / `real` 模式下真实源拉不到就标注 `nosource`（价格 0），持仓市值按 0 计——这是刻意设计，避免假数据污染盈亏。想离线也有曲线可看（演示 / 截图），把「设置」里的数据源模式显式改为 `sim`。每条行情的 `status` 字段（`ok`/`sim`/`stale`/`nosource`）会如实反映数据来源。

**Q：没有 DeepSeek API Key 能用 AI 分析吗？**
不能。AI 研判必须先到「设置 → AI 分析」配置 DeepSeek API Key（V4 系列模型），否则点击分析会提示先去配置、不会产生任何分析结论。配置 Key 后若调用失败，才会降级到本地启发式引擎并明确标注「已降级」。
