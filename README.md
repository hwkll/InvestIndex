...
> v1.1.0 · 把加密货币、基金、黄金、股票放进同一个视图，本地跑、数据不出机器、单文件部署。
>
> 应用版本号以 `internal/api/api.go` 的 `Version` 常量为唯一真相源（当前 `v1.1.0`），README 与之保持一致；请勿在多处硬编码版本号。
...
| 模块 | 能力 |
|------|------|
| **仪表盘** | 总资产 / 浮动盈亏 / 今日盈亏 / 现金占比 四大 KPI（加载时**脉冲骨架屏**占位）；资产分布环形图；7d / 30d / 90d / 全部 资产趋势折线图（支持**基准对比叠加**）；分类概览表（**实时增量刷新**，含今日最强/最弱）；**行情状态徽标**（真实 / 暂无源）；**页面切换淡入淡出过渡** |
...
| **资产详情** | ECharts K 线（日 / 周 / 月）+ MA5 / MA20 / MA60 + 成交量副图 + 区间缩放；MACD、RSI、KDJ、布林带等技术指标 KPI（K 线来自上游真实日线源，详见「行情数据源」章节） |
| **交易流水** | 买入 / 卖出 双向记录（数量、价格、手续费、备注）；支持筛选、分页、编辑、删除；CSV 双向导入导出 |
| **AI 研判** | 接入 DeepSeek V4 对单个标的或整体组合做分析，输出信号（买入 / 卖出 / 观望）、理由、风险与建议动作；**必须先配置 DeepSeek API Key**，未配置时提示先去设置，不进行分析 |
...
- 唯一的出网请求是你主动开启的行情源（Binance / 新浪财经 / 东方财富）和可选的 DeepSeek API
- **诚实优先**：`auto` / `real` 模式下真实源拉不到就如实标注 `nosource`（价格 0），**绝不用模拟价冒充真实行情**污染盈亏；所有数据均来自真实上游，无任何模拟 / 合成逻辑
...
|------|--------|------|
| `PORT` | `7788` | HTTP 监听端口 |
| `HOST` | *(空)* | 监听地址。**留空默认 `127.0.0.1`**，即仅本机可访问（安全默认）。设为 `0.0.0.0` 等才监听所有网卡（局域网可访问），此时务必同时设置访问口令并前置 TLS 反向代理 |
| `INVESTHUB_DB` | `./data/investhub.db` | SQLite 数据库路径，主密钥存放于同目录 |
| `INVESTHUB_SECRET` | *(空)* | 自定义加密主密钥；留空则使用 `data/master.key`（自动创建） |
| `POLL_INTERVAL` | *(已移除)* | 旧环境变量**已弃用**；行情 / 汇率刷新间隔现改为设置页「刷新频率」里的 `poll_interval` / `fx_refresh_interval`（存数据库、即时生效、无需重启） |
| `CORS_ALLOW_ORIGIN` | *(空)* | 跨域白名单。**默认留空 = 不发任何 CORS 头**（同源 SPA 无需跨域，且避免把投资数据暴露给任意网页）。仅当确需从别的源（如独立前端开发服务器）调用 API 时，设为那个**固定受信源**（如 `https://app.example.com`）；绝不会反射请求方的 Origin |
...
| `data_source_mode` | `auto` | 行情数据源模式：<br>`auto` = 优先真实源，失败即标注 `nosource`<br>`real` = 只用真实源，失败即标注 `nosource`（与 `auto` 行为一致，无模拟器降级） |
| `poll_interval` | `30` | 行情轮询间隔（秒），设置页「刷新频率」可调，**保存即生效无需重启**；范围 5–600（上限 10 分钟，防止错失每日快照） |
| `fx_refresh_interval` | `1800` | 多币种汇率刷新间隔（秒），设置页「刷新频率」可调，**保存即生效无需重启**；范围 60–86400（1 分钟至 1 天） |
...
| `deepseek_model` | `deepseek-v4-flash` | 使用的模型名（DeepSeek V4；`deepseek-chat` 已于 2026-07-24 弃用） |
| `webhook_url` | *(空)* | 提醒 Webhook 地址（任意兼容 JSON POST 的服务，如企业微信 / 钉钉 / 自定义），提醒渠道含 `webhook` 时触发。**仅支持 https**（默认）；服务端做 SSRF 防护（禁私网 / 回环 / 重定向，且不经代理）。地址以 AES-256-GCM 加密存储，前端仅见是否已配置。确需纯内网 http 接收端时设 `INVESTHUB_WEBHOOK_ALLOW_HTTP=1`（仍强制 IP 校验） |
...
| 类型 | 数据源 | 标识 | 计价 |
|------|--------|------|------|
| 加密货币 | Binance 24h ticker（无需 key），失败再试 CoinGecko | `binance` | USD |
| A 股 / 场内 ETF | 新浪财经行情接口 | `sina` | CNY |
| 现货黄金 | 新浪 `gds_AUTD`（上海金交所 Au(T+D)，CNY/克） | `sge` | CNY |
| 伦敦金 | 新浪 `hf_XAU`（USD/盎司） | `sina` | USD |
...
> ```bash
> HTTP_PROXY=http://127.0.0.1:<你的代理端口> HTTPS_PROXY=http://127.0.0.1:<你的代理端口> ./investhub
> ```
...
│   ├── ai/             ai.go   # DeepSeek 调用（需配置 Key）+ 调用失败兜底启发式 + 并发限流
...
1. **必须配置 `deepseek_api_key`** 才能使用 AI 研判。未配置时 `Analyze` 直接返回 `ErrNotConfigured`，接口返回 `40301`，前端展示「去设置」提示并禁用分析按钮，**不做任何分析、不写入分析记录**。
2. 配置了 Key 且调用成功 → 调用 DeepSeek（默认 `deepseek-v4-flash`），最多 2 个并发。
3. 配置了 Key 但调用失败（网络 / 鉴权 / 限流等）→ **直接返回错误**，接口返回 `500` 并附带失败原因（如 `DeepSeek HTTP 401`），前端以错误提示展示，**不做任何兜底分析、不写入分析记录**。

> 历史版本在未配置 Key 或调用失败时会静默使用本地启发式引擎「假装」AI 分析；现版本已改为明确要求配置 Key、调用失败直接报错，避免误导。

### 行情状态透明度

每条行情带 `status` 字段：`ok`（来自真实源）/ `nosource`（真实源不可达）。仪表盘顶部在存在 `nosource` 资产时显示提示徽标，让用户清楚当前数据的可信度。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/dashboard/summary` | 全局汇总（KPI + 分类明细 + 今日最强/最弱 `top` + `quoteNosourceCount`） |
...
| 事件 | 触发时机 | 载荷 |
|------|----------|------|
| `quote` | 每轮行情轮询完成（默认 30s） | 单条行情 `{assetId, price, prevClose, chgPct, currency, sourceTime, status}`（`status` ∈ `ok`/`nosource`） |
| `alert` | 提醒规则命中 | 提醒事件对象 |
...
| 环节 | 做法 |
|------|------|
| 访问口令 | **scrypt** 哈希后存储，永不明文落盘；改旧口令与登录**共享失败锁** |
| 敏感配置 | **AES-256-GCM** 加密，主密钥 `data/master.key` 权限 `0600`（可用 `INVESTHUB_SECRET` 覆盖） |
| 会话 | HttpOnly Cookie，30 天**滑动过期**（SSE 长连接不触发续期），登出即失效；清除口令会吊销全部会话；经 TLS 时自动加 `Secure` 标志 |
| 暴力破解 | 连续 **5 次失败锁定 5 分钟**（按客户端 IP 计，覆盖登录与改口令） |
| 请求体 | 限制 **20 MB**，防止内存打爆 |
| 设置更新 | 仅接受白名单内已知键，静默跳过未知 key |
| 备份导入 | JSON 列名经 `^[a-zA-Z_][a-zA-Z0-9_]*$` 正则校验，防 SQL 注入；导入前自动落回滚备份（权限 `0600`），且**永不覆写 `access_pin_hash`** |
| 备份导出 | 自动剔除 `access_pin_hash` 及任何 `*_hash`/`*_secret` 键，避免口令哈希随备份外泄 |
| CORS | **默认关闭**（不反射 Origin、不带凭据）；仅 `CORS_ALLOW_ORIGIN` 显式配置固定受信源时开放 |
| 数据库文件 | `data/` 目录权限 `0700`，`investhub.db`（含 `-wal`/`-shm`）权限 `0600`，防止同机其他账户读取明文投资数据 |
| 监听地址 | 默认 `127.0.0.1:7788`，即**仅本机**（安全默认） |
| Webhook 出站 | **仅允许 https**（默认），目标解析后 IP 若属回环 / 私网 / 链路本地 / 未指定（含云元数据 `169.254.169.254`）一律拒绝；禁止重定向；请求**直连目标、不走任何 HTTP 代理**（`Proxy: nil`，防代理成为 SSRF 跳板）；失败时仅向客户端返回「发送失败」泛化文案，原因只记服务端日志 |
| Webhook 凭据 | `webhook_url` 以 **AES-256-GCM** 落盘加密（与 API Key / SMTP 口令同级），`/settings` 列表仅返回 `has_value` 掩码、不返回明文；前端「留空 = 不修改」避免保存其他设置时误清已加密地址 |

> ⚠️ **安全默认值**：自 v1.1.0 起，未设置 `HOST` 时仅监听回环地址 `127.0.0.1`，同 Wi-Fi 下的其他设备**无法**直接访问你的投资数据。如需在局域网/公网访问，必须**显式**设置 `HOST` 且同时：① 在「设置」页设置访问口令开启鉴权；② 前置 Nginx/Caddy 等反向代理并启用 TLS。服务启动时会对此类暴露打印醒目告警。
>
> ```bash
> HOST=127.0.0.1 ./investhub      # 默认且最安全：只允许本机访问
> # HOST=0.0.0.0 ./investhub      # 仅在确需局域网访问时，并配好口令 + TLS
> ```
>
> 如果这台机器在公共网络上，**两者建议都做**。
...
```bash
INVESTHUB_DB=/tmp/dev.db ./investhub
```

调试时可在设置页「刷新频率」把行情刷新间隔调到 5 秒，配临时数据库不污染正式数据。
...
**健壮性**
- 行情 / 汇率刷新间隔改为设置页可配置（`poll_interval` 5–600s、`fx_refresh_interval` 60–86400s），保存即热重载、无需重启
...
**Q：必须联网吗？**
不必须，但断网时**不会**用模拟价冒充真实行情。默认 `auto` / `real` 模式下真实源拉不到就标注 `nosource`（价格 0），持仓市值按 0 计——这是刻意设计，避免假数据污染盈亏；所有数据均来自真实上游，无任何模拟 / 合成逻辑。每条行情的 `status` 字段（`ok`/`nosource`）会如实反映数据来源。

**Q：没有 DeepSeek API Key 能用 AI 分析吗？**
不能。AI 研判必须先到「设置 → AI 分析」配置 DeepSeek API Key（V4 系列模型），否则点击分析会提示先去配置、不会产生任何分析结论。配置 Key 后若调用失败（网络 / 鉴权 / 限流等），服务端**直接返回错误 500**并附失败原因，不做任何本地启发式兜底，避免误导。
